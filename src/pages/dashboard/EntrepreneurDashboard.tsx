...
export const EntrepreneurDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        if (user) {
            const reqRes = await apiClient.get('/collaboration/requests/incoming');
            setCollaborationRequests(reqRes.data);
            
            const meetRes = await apiClient.get('/meetings/my-calendar');
            setUpcomingMeetingsCount(meetRes.data.filter((m: any) => m.status === 'accepted').length);
        }
    };
    fetchData();
  }, [user]);
